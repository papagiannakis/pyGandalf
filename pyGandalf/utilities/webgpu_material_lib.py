...
class WebGPUMaterialLib(object):
    ...
    def rebuild(cls, name: str):
        material_instance: MaterialInstance = cls.instance.materials[name]
        shader_data = WebGPUShaderLib().get(material_instance.data.base_template)
        material_data = material_instance.data
        ...
        for uniform_name in other.keys():
            ...
            match other_data['type']['name']:
                case 'texture_2d<f32>':
                    # Retrieve texture.
                    texture_inst: TextureInstance = WebGPUTextureLib().get_instance(material_data.textures[texture_index])
        ...
        cls.instance.cached_materials[shader_data] = MaterialInstance(name, material_data, material_instance.descriptor, shader_data.shader_module, shader_data.pipeline_layout, bind_groups, uniform_buffers, uniform_buffer_types, storage_buffers, storage_buffer_types, other_uniforms, [])
        cls.instance.materials[name] = MaterialInstance(name, material_data, material_instance.descriptor, shader_data.shader_module, shader_data.pipeline_layout, bind_groups, uniform_buffers, uniform_buffer_types, storage_buffers, storage_buffer_types, other_uniforms, [])
